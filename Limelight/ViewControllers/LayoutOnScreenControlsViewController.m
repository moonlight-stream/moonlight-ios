...
- (IBAction)undoTapped:(id)sender {
    ...
    if ([self.layoutOSC.layoutChanges count] > 0) { //check if there are layout changes to roll back to
        ...
        OnScreenButtonState *buttonState = [self.layoutOSC.layoutChanges lastObject];
        CALayer *buttonLayer = [self.layoutOSC buttonLayerFromName:buttonState.name];
...
- (IBAction)saveTapped:(id)sender {
    ...
    [inputNameAlertController addAction:[UIAlertAction actionWithTitle:@"Save" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {   //add save button to pop up notification
        ...
        if ([enteredProfileName isEqualToString:@"Default"]) {  //don't user user overwrite the 'Default' profile
            ...
            UIAlertController * savedAlertController = [UIAlertController alertControllerWithTitle: [NSString stringWithFormat:@""] message: [NSString stringWithFormat:@"%@ profile saved and set as your active in-game controller profile layout", enteredProfileName] preferredStyle:UIAlertControllerStyleAlert];  //Let user know this profile is now the selected controller layout
...
- (IBAction)loadTapped:(id)sender {
    ...
    vc.didDismissOSCProfilesTVC = ^() {   //block that will be called when the 'OSCProfilesTableViewController' view controller is dismissed. code will hide/show and move each button to their appropriate position
